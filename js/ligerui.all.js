...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
	    ...
	    if (p.initValue != null) g.valueField[0].value = p.initValue;
            ...
            {
                if (!p.cancelable || p.disabled || p.readonly) return;
        ...
        _setValue: function (value,text)
        {
            ...
            else
            {
                ...
                if (value != null) {
                    ...
                    for (var i = 0; i < targetdata.length; i++) {
                        $("table.l-table-checkbox tr[value=" + targetdata[i] + "] :checkbox", g.selectBox).each(function () { this.checked = true; $("table.l-table-checkbox tr[value=" + targetdata[i] + "] a").addClass("l-checkbox-checked");});
                    }
                }
            }
        },
        ...
        setData: function (data)
        {
            ...
            g.emptyRow = null; //xiaobo add
            var out = []; 
            if (p.emptyText && !g.emptyRow  && p.emptyText!='no') //xiaobo add
            { 
                g.emptyRow = {};
            ...
            {
                g.addRowButton = $('<div class="l-box-select-add"><a href="javascript:void(0)" class="link"> </a></div>'); 
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            *************/
            ...
            g.link.hover(function ()
            {
                if (!p.cancelable || p.disabled) return;
                this.className = "l-trigger-hover";
            }, function ()
            {
                if ( p.disabled) return;
        ...
        bulidContent: function ()
        {
            ...
            $("tr", this.body.tbody).each(function (i, tr)
            {
				...
				var flag = 0;
				if (g.start.month == g.end.month)
				{
					if ((g.currentDate.year == g.start.year && 
						g.currentDate.month == g.start.month && id + 1 >= g.start.date && (id <= g.end.date - 1)) )
					{
						flag = 2;
						for (k in g.arr)
						{
							if ((g.temp.year == g.arr[k].year) && (g.temp.month == g.arr[k].month) && (g.temp.date == g.arr[k].date))
							{
								flag = 1;
							}
						}
				...
				{
					if ((g.currentDate.year == g.start.year && 
						g.currentDate.month == g.start.month && id + 1 >= g.start.date && (id <= monthDayNum - 1)) || (g.currentDate.year == 
						g.end.year && g.currentDate.month == g.end.month && id + 1 <= g.end.date && (id >= 0)))
					{
						flag = 2;
						for (k in g.arr)
						{
                            ...
                            id + 1 == g.now.date) || flag > 0)
                    {
						...
						if (flag == 1)
						{
							$(td).addClass("l-box-dateeditor-done");
						}
						else if (flag == 2)
						{
                        	$(td).addClass("l-box-dateeditor-undone");
						}
						else
						{
    ...
    $.ligerui.controls.Form.ligerExtend( $.ligerui.core.UIComponent, {
        ...
        _preSetFields: function ( fields )
        {
            ...
            $( p.fields ).each( function ( i, field )
            {
                ...
                if ( field.newline == null ) field.newline = false;
...
(function ($)
{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("        <div class='l-panel-topbar' style='display:none'><div class='l-panel-topbarinner' style='width:100%'></div></div><div class='l-clear'></div>");
        ...
        {
            var g = this, p = this.options;
            if (p.xb_showloading==false) show=false;
            this.gridloading[show ? 'show' : 'hide']();
        },
         xb_showLoading: function (show)
        {
            var g = this, p = this.options;
            p.xb_showloading=show;
        },
        _createEditor: function (editorBuilder, container, editParm, width, height)
        {
        ...
        */
        ...
        _buildPager: function ()
        {
            ...
            if (!p.total || p.total=="0") p.total = 0;
...
* http://ligerui.com
*  
* Author wangmian 2014 [ gd_star@163.com ] 
* 
*/
(function ($)
{
        ...
        addItem: function (item)
        {
            ...
            else
            {
				...
				}, function ()
				{
					...
					if (item.pos && item.pos =='right')ditem.css("float","right");
